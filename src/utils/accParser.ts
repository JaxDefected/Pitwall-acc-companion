...
export function parseAccSetup(rawJson: any, defaultFilename: string = "setup.json"): NormalizedAccSetup {
  ...
  if (tyresSection) {
    ...
    if (Array.isArray(rawPressures) && rawPressures.length === 4) {
      const basePsi = car?.tyrePressure_base ?? 20.3;
      const psiStep = car?.tyrePressure_step ?? 0.1;
      normalized.tyrePressures = rawPressures.map(step => 
        step > 100 ? step : Math.round((basePsi + step * psiStep) * 10) / 10
      );
    }
  }
  ...
  if (aeroSection) {
    ...
    if (Array.isArray(rhVal)) {
      const fBase = car?.rideHeightFront_base ?? 50;
      const rBase = car?.rideHeightRear_base ?? 50;
  ...
  if (dampersSection) {
    normalized.bumpSlow = getNestedVal(dampersSection, ["bumpSlow"]) ?? normalized.bumpSlow;
    normalized.bumpFast = getNestedVal(dampersSection, ["bumpFast"]) ?? normalized.bumpFast;
    normalized.reboundSlow = getNestedVal(dampersSection, ["reboundSlow"]) ?? normalized.reboundSlow;
    normalized.reboundFast = getNestedVal(dampersSection, ["reboundFast"]) ?? normalized.reboundFast;
  }
